        ...
        branches: [
          StatefulShellBranch(routes: [
            GoRoute(
                ...
                redirect: (context, state) {
                  bool isContainCompanies =
                      Dependencies.contain<List<CompanyEntity>>();
                  bool isContainToken = Dependencies.get<SharedPrefService>()
                      .contains(SharedPrefService.token);
                  if (!isContainCompanies || isContainToken) {
                    return initializer;
                  }
                },
                pageBuilder: (context, state) =>
                    NoTransitionPage(child: LoginScreen.page()))
          ]),
          StatefulShellBranch(routes: [
            GoRoute(
                path: register,
          ...
          StatefulShellBranch(routes: [
            GoRoute(
                path: initializer,
                pageBuilder: (context, state) =>
                    NoTransitionPage(child: InitialiserScreen.page()))
          ]),
        ]),
    GoRoute(
        path: home,
