...
class RepositoryIml extends Repository {
  ...
  Future<List<CompanyEntity>> getCompanies() async {
    ...
    List<CompanyEntity> companiesE =
        companies.map((e) => e.toEntity()).toList();
    return companiesE;
  }
  ...
  Future<List<EntrepotEntity>> getListEntropot(
    ...
    List<EntrepotModel> entrepotM =
        await apiClient.getListEntropot(society: society, idType: idType);
    List<EntrepotEntity> entrepotE =
        entrepotM.map((e) => e.toEntity()).toList();
    return entrepotE;
  }
  ...
  Future<List<ProductEntity>> getAllProductsByEntrepot(int idEntrepot) async {
    List<ProductModel> productsM =
        await apiClient.getAllProductsByEntrepot(idEntrepot);
  ...
  Future<List<ChargementEntity>> getAllChargement(int entrepot) async {
    List<ChargementModel> chargementsM =
        await apiClient.getAllChargement(entrepot);
    List<ChargementEntity> chargementE =
        chargementsM.map((e) => e.toEntity()).toList();
    return chargementE;
  }
  ...
  Future<List<ActivityClientEntity>> getClientActivities() async {
    List<ActivityClientModel> activitiesM =
        await apiClient.getClientActivities();
    List<ActivityClientEntity> activitiesE =
        activitiesM.map((e) => e.toEntity()).toList();
    return activitiesE;
  }
  ...
  Future<CommandEntity> livrerCommand(LivComdReqEntity request) async {
    CommandModel command =
        await apiClient.livrerCommand(LivComdReqModel.toModel(request));
    return command.toEntity();
  }
  ...
  Future<List<LivraisonEntity>> getAllLivraison(int company) async {
    ...
    List<LivraisonEntity> livraisonE =
        livraisonM.map((e) => e.toEntity()).toList();
    return livraisonE;
  }
  ...
  Future<List<PaymentModeEntity>> getPayments() async {
    ...
    List<PaymentModeEntity> paymentsE =
        paymentsM.map((e) => e.toEntity()).toList();
    return paymentsE;
  }

  @override
  Future<List<FactureEntity>> getFacturesNotPaid(
      int client, int company) async {
    List<FactureModel> facturesM =
        await apiClient.getFacturesNotPaid(client, company);
  ...
  Future<List<EntrepotEntity>> getEntrepotsByUser() async {
    ...
    List<EntrepotEntity> entrepotE =
        entrepots.map((e) => e.toEntity()).toList();
    return entrepotE;
  }
  ...
  Future<List<ProductEntity>> getProductsByEntrepot(int entrepot) async {
    List<ProductModel> products =
        await apiClient.getProductsByEntrepot(entrepot);
  ...
  @override
  Future<List<LivraisonEntity>> getLivraisonsByClient(
      int client, int company) async {
    List<LivraisonModel> livraisons =
        await apiClient.getLivraisonsByClient(client, company);
    List<LivraisonEntity> livraisonE =
        livraisons.map((e) => e.toEntity()).toList();
    return livraisonE;
  }
  ...
  @override
  Future<List<LivraisonEntity>> getLivraisonByDate(
      int comany, String date) async {
    List<LivraisonModel> livraisons =
        await apiClient.getLivraisonsByDate(comany, date);
    List<LivraisonEntity> livraisonE =
        livraisons.map((e) => e.toEntity()).toList();
    return livraisonE;
  }
  ...
  // Reception management
   @override
  Future<ReceptionEntity> createReception(ReceptionRequest request) async {
    final model = await apiClient.createReception(request.toJson());
    return model.toEntity();
  }
 
  @override
  Future<List<ReceptionEntity>> getReceptions() async {
    final models = await apiClient.getReceptions();
    return models.map((m) => m.toEntity()).toList();
  }
 
  @override
  Future<ReceptionDetailEntity> createReceptionDetail(int receptionId, ReceptionDetailRequest request) async {
    final model = await apiClient.createReceptionDetail(receptionId, request.toJson());
    return model.toEntity();
  }
 
  @override
  Future<List<ReceptionDetailEntity>> getReceptionDetails() async {
    final models = await apiClient.getReceptionDetails();
    return models.map((m) => m.toEntity()).toList();
  }
 
  @override
  Future<String?> getNextRef() async {
    final model = await apiClient.getNextRef();
    return model.nextref;
  }
 
  @override
  Future<ArticleUnitEntity> getArticleUnit(int idArticle) async {
    final model = await apiClient.getArticleUnit(idArticle);
    return ArticleUnitEntity(tva: model.tva, label: model.label, unite: model.unite);
  }
 
  @override
  Future<List<FournisseurEntity>> getFournisseurs() async {
    final models = await apiClient.getFournisseurs();
    return models.map((m) => m.toEntity()).toList();
  }
 
  @override
  Future<List<EntrepotReceptionEntity>> getEntrepots(int idSociete) async {
    final models = await apiClient.getEntrepots(idSociete);
    return models.map((m) => m.toEntity()).toList();
  }
 
  @override
  Future<List<ArticleEntity>> getArticles(int idSociete) async {
    final models = await apiClient.getArticles(idSociete);
    return models.map((m) => m.toEntity()).toList();
  }
 
  @override
  Future<List<BateauEntity>> getBateaux() async {
    final models = await apiClient.getBateaux();
    return models.map((m) => m.toEntity()).toList();
  }
 
  @override
  Future<List<UniteEntity>> getUnites() async {
    final models = await apiClient.getUnites();
    return models.map((m) => m.toEntity()).toList();
  }
}
