...
class CommandDetailScreen extends StatelessWidget {
  ...
  CommandDetailScreen({required this.commands});
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          InfoItem(
            name: "Date livraison",
            value: commands.dateCommand?.formattedDateFr,
          ),
          InfoItem(
            name: "total HT",
            value: commands.totalHt?.toString()??"-",
          ),
          InfoItem(
            name: "total TTC",
            value: commands.totalTTC?.toString()??"-",
          ),
          InfoItem(
            name: "total TVA",
            value: commands.totalTva?.toString()??"-",
          ),
          Padding(
              ...
              children: [
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                      color: commands.status?.color?.toColor,
                      borderRadius: BorderRadius.circular(7)
                  ),
                  child: Text(commands.status?.status??"",style: GoogleFonts.aBeeZee(color:Colors.white,fontSize:15,fontWeight:FontWeight.bold),),
                ),
              ],
            ),
          ),
                    ...
                    Expanded(
                      child: Padding(
                        ...
                        child: ListView.builder(
                            itemCount: commands.details?.length??0,
                            itemBuilder: (context,index){
                              CommandDetailEntity command=commands.details!.elementAt(index);
                              return Container(
                                ...
                                child: Column(
                                  children: [
                                    ...
                                    InfoItem(
                                      name: "Quantité",
                                      value: command.quantity?.toString()??"-",
                                    ),
                                    InfoItem(
                                      name: "Quantité restante",
                                      value: command.qtyRestante?.toString()??"-",
                                    ),
                                    InfoItem(
                                      name: "Total HT",
