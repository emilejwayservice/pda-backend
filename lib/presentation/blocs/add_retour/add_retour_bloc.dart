...
class AddRetourBloc extends Bloc<AddRetourEvent, AddRetourState> {
  ...
  FutureOr<void> _fetchData(FetchData event, Emitter<AddRetourState> emit)async{
    try{
      ...
      emit(state.copyWith(fetchDataStatus: AppStatus.success,clients: clients,entrepots: entrepots));
  ...
  FutureOr<void> _selectEntrepot(SelectEntrepot event, Emitter<AddRetourState> emit) {
    emit(state.copyWith(selectedEntrepot: event.entrepot));
  }

  FutureOr<void> _selectClient(SelectClient event, Emitter<AddRetourState> emit) async{
    try{
      emit(state.copyWith(fetchProductsStatus: AppStatus.loading,selectedClient: event.client));
      Repository repository=Dependencies.get<Repository>();
      int company=Dependencies.get<SharedPrefService>().getValue(SharedPrefService.company, 0);
      List<LivraisonEntity> livraisons=await repository.getLivraisonsByClient(event.client.id!, company);
      emit(state.copyWith(fetchProductsStatus: AppStatus.success,livraisons: livraisons));
    }on NetworkConnectivityException catch(ex){
  ...
  FutureOr<void> _valider(Valider event, Emitter<AddRetourState> emit) async {
    try{
      List<ProductEntity> selectedProducts=(state.selectedLivraison?.details??[]).where((element) => element.product!.isSelected).map((e) => e.product!).toList();
      if(selectedProducts.isEmpty){
        emit(state.copyWith(validerStatus: AppStatus.warning,error: "Vous devez sélectionner des produits."));
        return;
      }
      List<ProductEntity> invalidProducts=selectedProducts.where((element) => (element.isSelected && (element.quantity==null|| element.quantity==0))).toList();
      if(invalidProducts.isNotEmpty){
        emit(state.copyWith(validerStatus: AppStatus.warning,error: "Vous devez saisir la quantité de chaque produit."));
        return;
      }
      if(state.selectedEntrepot==null){
        emit(state.copyWith(validerStatus: AppStatus.warning,error: "Vous devez sélectionner un entrepôt."));
        return;
      }
  ...
  FutureOr<void> _selectLivraison(SelectLivraison event, Emitter<AddRetourState> emit) {
