...
class LoginBloc extends Bloc<LoginEvent, LoginState> {
  ...
  FutureOr<void> _login(Login event, Emitter<LoginState> emit) async{
    try{
      ...
      UserEntity user=await repository.login(event.email,event.password,event.company);
      Dependencies.put(user);


      ApiClient apiClient=ApiClientIml(baseUrl: baseUrl,token: user.token);

      Repository newRepository=(Dependencies.get<Repository>() as RepositoryIml).copyWith(apiClient: apiClient);
